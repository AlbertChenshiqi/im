...
func TestExtractToken(t *testing.T) {
	r := httptest.NewRequest(http.MethodGet, "/v1/ws?token=abc", nil)
	if ExtractToken(r) != "abc" {
		t.Fatal("query token")
	}
	r2 := httptest.NewRequest(http.MethodGet, "/v1/ws", nil)
...
func TestAuthenticateWS(t *testing.T) {
	...
	r := httptest.NewRequest(http.MethodGet, "/v1/ws?token="+tok, nil)
