...
func (r *RealtimeMessage) Run(ctx context.Context) {
	...
	log.Println("[transfer] realtime-message started")
...
func (r *RealtimeMessage) fanout(ctx context.Context, evt events.MessageSendEvent) {
	...
	for _, uid := range targets {
		...
		if err := events.PublishGatewayPush(ctx, r.svc.GatewayPush, uid, frame); err != nil {
			log.Printf("[transfer] gateway message uid=%d msg=%d: %v", uid, evt.MsgID, err)
		}
	}
}
...
func (r *RealtimeMessage) targets(ctx context.Context, evt events.MessageSendEvent) []int64 {
	...
	var out []int64
	err := forEachGroupMember(ctx, r.svc.DB, evt.GroupID, r.svc.Config.Transfer.MemberBatch, func(uid int64) {
		if uid > 0 && uid != evt.SenderID {
			out = append(out, uid)
		}
	})
	if err != nil {
		log.Printf("[transfer] realtime list members gid=%d: %v", evt.GroupID, err)
		return nil
	}
	return out
}
