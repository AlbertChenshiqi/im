...
| `custom` | 自定义消息（`msg_type=custom` 或 `custom_*`） | message/rpc | 同上 |
| `recall` | 撤回（`msg_type=recall`） | message/rpc | 同上（预留） |
...
| Make 目标 | 说明 |
|-----------|------|
| `make up` / `make k8s-up` | 一键部署（Postgres / Redis / RocketMQ + 全部微服务） |
...
```bash
make run-all             # 本机启动各 RPC/API（连 localhost:5432/6379/9876）
```

kind 集群已将 Postgres / Redis / RocketMQ NameServer **NodePort 映射到本机标准端口**，与 `write-etc.sh` 配置一致。非 kind 集群再用 `make k8s-forward-infra` 做端口转发。
